    ...
    fields: {
      ...
      'error.stack': {
        ...
        htmlFormatter: (value: string) => {
          const cls = 'text-xs bg-gray-800 p-3 rounded overflow-x-auto'
          const fullCls = `${cls} whitespace-pre-wrap leading-relaxed`
          const html = `<div class=""><div class="mb-1">` +
            `<strong>Stack Trace:</strong></div>` +
            `<pre class="${fullCls}">${value}</pre></div>`
          return html
        },
    ...
    fields: {
      ...
      jsonError: {
        ...
        htmlFormatter: (value: any) => {
          ...
          const html = `<div class="mb-3"><div class="mb-1">` +
            `<strong>JSON Error Details:</strong></div>` +
            `<pre class="${fullCls}">${jsonStr}</pre></div>`
          return html
        },
    ...
    fields: {
      ...
      details: {
        ...
        htmlFormatter: (value: any) => {
          ...
          const jsonStr = JSON.stringify(value, null, 2)
          const html = `<div class="mb-3"><div class="mb-1">` +
            `<strong>Channel Error Details:</strong></div>` +
            `<pre class="${fullCls}">${jsonStr}</pre></div>`
          return html
        },
  ...
  stimulus: {
    ...
    fields: {
      ...
      positioningIssues: {
        ...
        htmlFormatter: (value: string[]) => {
          const cls = 'text-xs list-disc list-inside bg-gray-800 p-3'
          const ulClass = `${cls} rounded space-y-1`
          const items = value
            .map((issue: string) => `<li class="leading-relaxed">${issue}</li>`)
            .join('')
          const html = `<div class="mb-3"><div class="mb-1">` +
            `<strong>Positioning Issues:</strong></div>` +
            `<ul class="${ulClass}">${items}</ul></div>`
          return html
        },
      ...
      elementInfo: {
        ...
        htmlFormatter: (value: any) => {
          ...
          const jsonStr = JSON.stringify(value, null, 2)
          const html = `<div class="mb-3"><div class="mb-1">` +
            `<strong>Element Info:</strong></div>` +
            `<pre class="${fullCls}">${jsonStr}</pre></div>`
          return html
        },
      ...
      suggestion: {
        ...
        priority: 3,
        htmlFormatter: (value: string) => {
          const cls = 'text-sm bg-blue-900 text-blue-200 p-3'
          const divClass = `${cls} rounded leading-relaxed`
          const html = `<div class="mb-3"><div class="mb-1">` +
            `<strong>💡 Suggestion:</strong></div>` +
            `<div class="${divClass}">${value}</div></div>`
          return html
        },
      ...
      details: {
        ...
        htmlFormatter: (value: any) => {
          ...
          const jsonStr = JSON.stringify(value, null, 2)
          const html = `<div class="mb-3"><div class="mb-1">` +
            `<strong>Detailed Information:</strong></div>` +
            `<pre class="${fullCls}">${jsonStr}</pre></div>`
          return html
        },
